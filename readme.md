...
请按照自己的环境配置相应选项，默认的 socket.io 的端口是 `6001` ，请注意！如果您是在阿里云服务器使用请注意开通 `6001` 端口！其它云服务商也是！如果您使用的 `https` 协议也请更换！
...
为了保证服务的稳定，你可以配合 `supervisor` ,在 `/etc/superviosor/conf.d/` 目录下面创建 `laravel-echoserver` :
...
同样如果再生产环境中使用请参考 [https://laravel.com/docs/5.5/queues#supervisor-configuration](https://laravel.com/docs/5.5/queues#supervisor-configuration) 这篇文章，配置 `supervisor` 使用！
